...
export default function NavbarComponent() {
  ...
  const userInitials = user?.name || user?.email.charAt(0).toUpperCase() || "U";
      ...
      return (
        <div className="flex items-center gap-3">
          <Dropdown placement="bottom-end">
            <DropdownTrigger>
              {/* The Avatar acts as the button to open the menu */}
              <Avatar
                as="button"
                className="transition-transform border border-1 h-10 w-10 hover:border-lime-300 hover:bg-lime-50 border-gray-200 bg-white/10 font-extrabold text-gray-900 text-lg hover:cursor-pointer"
                name={userInitials}
              />
            </DropdownTrigger>
